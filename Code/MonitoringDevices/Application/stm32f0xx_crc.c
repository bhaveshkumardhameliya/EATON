...
/**
  ...
  * @retval None
  */
void CRC_ReverseOutputDataCmd(uint8_t NewState)
{
  if (NewState != 0)
  {
...
/**             
  ...
  */  
uint32_t CRC_Handle_16(uint16_t pBuffer[], uint32_t BufferLength)
{
  ...
  {
    CRC->DR = ((uint32_t)pBuffer[2U*i]<<16U) | (uint32_t)pBuffer[2U*i+1];     
  }
  if ((BufferLength%2U) != 0U)
  {
       *(uint16_t volatile*) (&CRC->DR) = pBuffer[2*i]; 
  }
...
/**             
  ...
  * @retval uint32_t CRC (returned value LSBs for CRC shorter than 32 bits)
  */
uint32_t CRC_Handle_8(uint8_t pBuffer[], uint32_t BufferLength)
{
  uint32_t i = 0U; /* input data buffer index */
   ...
   {
      CRC->DR = ((uint32_t)pBuffer[4U*i]<<24U) | ((uint32_t)pBuffer[4U*i+1]<<16U) | ((uint32_t)pBuffer[4U*i+2]<<8U) | (uint32_t)pBuffer[4U*i+3];      
   }
   ...
   if ((BufferLength%4U) != 0U)
   {
     ...
     {
       *(uint8_t volatile*) (&CRC->DR) = pBuffer[4*i];
     }
     if  (BufferLength%4U == 2U)
     {
       *(uint16_t volatile*) (&CRC->DR) = ((uint32_t)pBuffer[4*i]<<8) | (uint32_t)pBuffer[4*i+1];
     }
     if  (BufferLength%4U == 3U)
     {
       *(uint16_t volatile*) (&CRC->DR) = ((uint32_t)pBuffer[4*i]<<8) | (uint32_t)pBuffer[4*i+1];
       *(uint8_t volatile*) (&CRC->DR) = pBuffer[4*i+2];       
     }
   }
